<?php 
/**
 * Class_Attendance Page Controller
 * @category  Controller
 */
class Class_AttendanceController extends SecureController{
	/**
     * Load Record Action 
     * $arg1 Field Name
     * $arg2 Field Value 
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'date', 	'name', 	'class', 	'roll', 	'status');
		...
		$records = $db->get('class_attendance', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'class_attendance', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'class_attendance' , false );
				}
				if($db->getLastError()){
	...
	function view( $rec_id = null , $value = null){
		...
		$record = $db->getOne( 'class_attendance', $fields );
		if(!empty($record)){
			render_json($record);
		}
		else{
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('class_attendance',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			$modeldata=transform_request_data($_POST);
			$db->where('id' , $rec_id);
			$bool = $db->update('class_attendance',$modeldata);
			if($bool){
				render_json($rec_id);
			}
			else{
			...
			$fields=array('id','date','name','class','roll','status');
			$db->where('id' , $rec_id);
			$data = $db->getOne('class_attendance',$fields);
			if(!empty($data)){
				render_json($data);
			}
			else{
	...
	function delete( $rec_ids = null ){
		...
		$bool = $db->delete( 'class_attendance' );
		if($bool){
			render_json( $bool );
		}
		else{
