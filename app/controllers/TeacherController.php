...
class TeacherController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'name', 	'designation', 	'slary', 	'doj');
		$limit = $this->get_page_limit(MAX_RECORD_COUNT); // return pagination from BaseModel Class e.g array(5,20)
		if(!empty($this->search)){
			$text=$this->search;
			$db->orWhere('id',"%$text%",'LIKE');
			$db->orWhere('name',"%$text%",'LIKE');
			$db->orWhere('designation',"%$text%",'LIKE');
			$db->orWhere('slary',"%$text%",'LIKE');
			$db->orWhere('doj',"%$text%",'LIKE');
		}
		...
		$records = $db->get('teacher', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'teacher', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'teacher' , false );
				}
				if($db->getLastError()){
	...
	function view( $rec_id = null , $value = null){
		...
		$record = $db->getOne( 'teacher', $fields );
		if(!empty($record)){
			render_json($record);
		}
		else{
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('teacher',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			$modeldata=transform_request_data($_POST);
			$db->where('id' , $rec_id);
			$bool = $db->update('teacher',$modeldata);
			if($bool){
				render_json($rec_id);
			}
			else{
