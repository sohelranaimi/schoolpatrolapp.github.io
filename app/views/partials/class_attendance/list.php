    <template id="class_attendanceList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Class Attendance</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/class_attendance/add'">
                            <i class="fa fa-plus"></i>
                            Add New Class Attendance
                            </router-link>
                        </div>
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/class_attendance">Class Attendance</router-link>
                                            </li>
                                        ...
                                        <table class="table" :class="tablestyle">
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    ...
                                                    <td><router-link :to="'/class_attendance/view/' +  data.id">{{data.id}}</router-link></td>
                                                    <td> {{ data.date }} </td>
                                                    <td> {{ data.name }} </td>
                                                    <td> {{ data.class }} </td>
                                                    <td> {{ data.roll }} </td>
                                                    ...
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/class_attendance/view/' + data.id">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/class_attendance/edit/' + data.id">
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="class_attendance/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var Class_AttendanceListComponent = Vue.component('class_attendanceList', {
		template: '#class_attendanceList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'class_attendancelist',
			},
			apipath : {
				type : String,
				default : 'class_attendance/list',
			},
			tablestyle: {
				type: String,
