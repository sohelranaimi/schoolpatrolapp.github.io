    <template id="despatchList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Despatch</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/despatch/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/despatch">Despatch</router-link>
                                            </li>
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    <th v-if="multicheckbox" class="td-checkbox">
                                                        <label>
                                                            <input type="checkbox" :value="data.DES_NUM" name="selectedid" v-model="selected" />
                                                        </label>
                                                    </th>
                                                    <th v-if="listsequence" class="td-sno">{{index + 1}}</th>
                                                    <td><router-link :to="'/despatch/view/' +  data.DES_NUM">{{data.DES_NUM}}</router-link></td>
                                                    <td> {{ data.DES_DATE }} </td>
                                                    <td> {{ data.DES_AMOUNT }} </td>
                                                    <td> {{ data.ORD_NUM }} </td>
                                                    <td> {{ data.ORD_DATE }} </td>
                                                    <td> {{ data.ORD_AMOUNT }} </td>
                                                    <td> {{ data.AGENT_CODE }} </td>
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/despatch/view/' + data.DES_NUM">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/despatch/edit/' + data.DES_NUM">
                                                            <i class="fa fa-edit"></i> 
                                                            </router-link>
                                                            <button  v-if="deletebutton" class="btn btn-outline-danger btn-sm" @click="deleteRecord(data.DES_NUM,index)" title="Delete This Record">
                                                                <span v-show="deleting != data.DES_NUM"><i class="fa fa-times"></i></span>
                                                                <clip-loader :loading="deleting == data.DES_NUM" color="#fff" size="14px"></clip-loader>
                                                            </button>
                                                        </div>
                                                    </th>
                                                </tr>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="despatch/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var DespatchListComponent = Vue.component('despatchList', {
		template: '#despatchList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'despatchlist',
			},
			apipath : {
				type : String,
				default : 'despatch/list',
			},
			tablestyle: {
				type: String,
