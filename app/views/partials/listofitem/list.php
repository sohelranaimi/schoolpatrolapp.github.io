    <template id="listofitemList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Listofitem</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/listofitem/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/listofitem">Listofitem</router-link>
                                            </li>
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    <th v-if="multicheckbox" class="td-checkbox">
                                                        <label>
                                                            <input type="checkbox" :value="data.ITEMCODE" name="selectedid" v-model="selected" />
                                                        </label>
                                                    </th>
                                                    <th v-if="listsequence" class="td-sno">{{index + 1}}</th>
                                                    <td><router-link :to="'/listofitem/view/' +  data.ITEMCODE">{{data.ITEMCODE}}</router-link></td>
                                                    <td> {{ data.ITEMNAME }} </td>
                                                    <td> {{ data.BATCHCODE }} </td>
                                                    <td> {{ data.CONAME }} </td>
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/listofitem/view/' + data.ITEMCODE">
                                                            <i class="fa fa-eye"></i> 
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/listofitem/edit/' + data.ITEMCODE">
                                                            <i class="fa fa-edit"></i> 
                                                            </router-link>
                                                            <button  v-if="deletebutton" class="btn btn-outline-danger btn-sm" @click="deleteRecord(data.ITEMCODE,index)" title="Delete This Record">
                                                                <span v-show="deleting != data.ITEMCODE"><i class="fa fa-times"></i></span>
                                                                <clip-loader :loading="deleting == data.ITEMCODE" color="#fff" size="14px"></clip-loader>
                                                            </button>
                                                        </div>
                                                    </th>
                                                </tr>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="" post-action="listofitem/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var ListofitemListComponent = Vue.component('listofitemList', {
		template: '#listofitemList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'listofitemlist',
			},
			apipath : {
				type : String,
				default : 'listofitem/list',
			},
			tablestyle: {
				type: String,
			...
			allSelected: function(){
				...
				if(this.allSelected == true){
					for (var i in this.records){
						var id = this.records[i].ITEMCODE;
